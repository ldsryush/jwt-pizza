...
export default function AdminDashboard(props: Props) {
  ...
  async function deleteUser(userId: string) {
    ...
    setUserList(await pizzaService.listUsers(userPage, 10, filterUserRef.current?.value ? `*${filterUserRef.current.value}*` : '*'));
  }
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {userList.users.map((user, index) => (
                          <tr key={index} className="bg-neutral-100">
                            <td className="px-6 py-4 whitespace-nowrap text-start text-sm text-gray-800">{user.name}</td>
